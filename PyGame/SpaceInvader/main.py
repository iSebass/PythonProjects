...
laser_sonido     = pygame.mixer.Sound('Recursos/laser.wav')
explosion_sonido = pygame.mixer.Sound('Recursos/explosion.wav')
golpe_sonido     = pygame.mixer.Sound('Recursos/golpe.wav')

explosion_list = []
for i in range(1,13):
    explosion = pygame.image.load(f'Recursos/explosion/{i}.png')
...
def barra_vida(frame,x,y,nivel):
    ...
    border   = pygame.Rect(x,y,fill,alto)
    color    = (255,0,55)
    fill     = pygame.Rect(x,y,fill,alto)
    pygame.draw.rect( frame, color, fill)
...
class Jugador(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        self.image        = pygame.image.load('Recursos/imagenes/A1.png').convert_alpha()
...
class Enemigos(pygame.sprite.Sprite):
    ...
    def __init__(self,x ,y):
        super().__init__()
        self.image       = pygame.image.load('Recursos/imagenes/E1.png').convert_alpha()
        self.rect        = self.image.get_rect()
    ...
    def disparar_enemigos(self):
        bala = Balas(self.rect.centerx, self.rect.bottom)
        grupo_jugador.add(bala)
...
class Balas(pygame.sprite.Sprite):
    ...
    def __init__(self, x,y):
        super().__init__()
        self.image        = pygame.image.load('Recursos/imagenes/B2.png').convert_alpha()
        self.rect         = self.image.get_rect()
        self.rect.centerx = x
        self.rect.y       = y
        self.velocidad    = -18
...
class Balas_enemgigos(pygame.sprite.Sprite):
    
    def __init__(self, x,y):
        super().__init__()
        self.image        = pygame.image.load('Recursos/imagenes/B1.png').convert_alpha()
...
while run:
    ...
    for hit in hits:
        player.vida -= 100
        enemgigos = Enemigos(10,10)
        grupo_jugador.add(enemgigos)
        grupo_enemigos.add(enemgigos)
        if player.vida <= 0:
            run= False
